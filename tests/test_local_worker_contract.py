...
def test_local_worker_rejects_low_system_memory(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("IDEOGRAM_MIN_SYSTEM_MEMORY_GB", raising=False)
    monkeypatch.setattr(local_worker, "_system_memory_gib", lambda: 31.0)

    with pytest.raises(RuntimeError, match="Increase system RAM"):
...
def test_local_worker_allows_enough_system_memory(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("IDEOGRAM_MIN_SYSTEM_MEMORY_GB", raising=False)
    monkeypatch.setattr(local_worker, "_system_memory_gib", lambda: 64.0)

    local_worker._assert_minimum_system_memory()


def test_local_worker_honors_explicit_system_memory_override(monkeypatch: pytest.MonkeyPatch) -> None:
